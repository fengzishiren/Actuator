...
namespace Script {
    ...
    struct Commands {
        ...
        static void set_cmd(Env &env, Instruction &pc) {
            assert(pc.params.size() == 2 && pc.params[0].type == kName, "语法错误", pc.pos);
            env.set_var(pc.params[0].content, get_val(env, pc.params[1]));
        }
        ...
        static void decl_cmd(Env &env, Instruction &pc) {
